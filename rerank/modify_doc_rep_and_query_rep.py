    ...
    if (idx + 1) % num == 0:
        with torch.no_grad():
            ...
            for dtext in dtexts:
                ...
                ori_d_ids = tokenizer.encode(dtext, add_special_tokens=False)
                combined_ids = set(doc_ids + ori_d_ids)
                ...
                for k in read_query[int(qid)]:
                    if k in combined_ids:
                        score += max_scores_map.get(k, 0) ** 2
                
                query_max_score_map = generate_token_scores(model, tokenizer, qtext, max_length=20, top_k=32000)
                query_top_tokens = get_top_tokens(query_max_score_map, top_n=50)
                for q_id, q_weight in query_top_tokens.items():
                    if q_id in combined_ids:
                        score += q_weight ** 2
